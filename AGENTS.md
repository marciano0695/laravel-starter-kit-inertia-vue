...
- Actions live in `app/Actions/{Model}/`, grouped by their related model. For example: `app/Actions/User/CreateUser.php`.
- Actions are named based on what they do, with no suffix.
...
- Create new actions with `php artisan make:action "{Model}/{Name}" --no-interaction`
...
final readonly class CreateWishListFavorite
{
...
- `wayfinder-development` — Activates whenever working with Wayfinder-generated TypeScript. Use when importing from @/wayfinder/, using routes, models, enums, form request types, Inertia page/shared-data types, broadcast channels/events, or env var types — keeping frontend and backend perfectly in sync.
...
# Laravel Wayfinder (Beta)

Wayfinder bridges Laravel and TypeScript — all generated files live under `resources/js/wayfinder` (alias `@/wayfinder`). It generates routes, models, enums, form request types, Inertia page/shared-data types, broadcast channels/events, and env var types.

- IMPORTANT: Activate `wayfinder-development` skill whenever working with any Wayfinder-generated TypeScript.
- Controllers: `import { PostController } from '@/wayfinder/App/Http/Controllers/PostController'; PostController.show({ post: 1 })`.
- Named Routes: `import posts from '@/wayfinder/routes/posts'; posts.show({ post: 1 })`.
- Models/Enums/Types: `import { App } from '@/wayfinder/types'` — use `App.Models.User`, `App.Enums.PostStatus`, `Inertia.Pages.Dashboard`.
- Enum constants: `import PostStatus from '@/wayfinder/App/Enums/PostStatus'; PostStatus.Published`.
- Parameter Binding: Detects route keys (`{post:slug}`) — `PostController.show({ slug: "my-post" })`.
- Query Merging: `PostController.index({ mergeQuery: { page: 2, sort: null } })` merges with current URL, `null` removes params.
- Form-safe routes: `PostController.update.form({ post: 1 })` — adds `_method` for PUT/PATCH/DELETE spoofing.
- Inertia: Use `.form()` with `<Form>` component or `form.submit(PostController.store())` with useForm.
- Generate: `php artisan wayfinder:generate --no-interaction` (no `--with-form` or `--skip-*` flags in beta).

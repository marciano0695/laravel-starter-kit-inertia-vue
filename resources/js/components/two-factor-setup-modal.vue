        ...
        <template v-if="!showVerificationStep">
          ...
          <template v-else>
            <div class="relative mx-auto flex max-w-md items-center overflow-hidden">
              <div
                class="relative mx-auto aspect-square w-64 overflow-hidden rounded-lg border border-border bg-gray-100"
              >
                <div
                  v-if="!qrCodeSvg"
            ...
            <div ref="pinInputContainerRef" class="relative w-full space-y-3">
              <div class="flex w-full flex-col items-center justify-center space-y-3 py-2">
                <PinInput id="otp" v-model="code" placeholder="○" type="number" otp>
                  <PinInputGroup>
                    <PinInputSlot v-for="(id, index) in 6" :key="id" autofocus :index="index" :disabled="processing" />
                  </PinInputGroup>
                </PinInput>
                <InputError :message="formErrors?.confirmTwoFactorAuthentication?.code" />
              </div>
